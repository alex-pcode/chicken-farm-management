  ...
  return (
    <div className={containerClasses}>
      ...
      <div className="relative">
        {/* Mobile card layout */}
        {responsive && (
          <div className="md:hidden divide-y divide-gray-200 dark:divide-gray-700" data-testid={testId ? `${testId}-mobile` : undefined}>
            {data.map((row, rowIndex) => (
              <div key={rowIndex} className="p-4 space-y-2">
                {columns.map((column, colIndex) => {
                  const value = row[column.key as keyof T];
                  const rendered = column.render ? column.render(value, row) : value as React.ReactNode;
                  // For the first column, render it prominently
                  if (colIndex === 0) {
                    return (
                      <div key={colIndex}>
                        {rendered}
                  ...
                  return (
                    <div key={colIndex} className="flex items-center justify-between gap-2">
                      <span className="text-xs font-medium text-gray-500 dark:text-gray-400 shrink-0">{column.label}</span>
                      <div className="text-right">{rendered}</div>
                    </div>
                  );
                })}
              </div>
