/**
 * Updated Onboarding Wizard Component - Multi-step setup flow with shared components
 * Guides users through flock setup with optional chicken entry
 * Uses BMad brand colors and shared form components
 */
...
export const OnboardingWizardUpdated: React.FC<OnboardingWizardProps> = ({
  onComplete,
  onBack,
  className = ''
}) => {
      ...
      >
        ...
        <div className="flex justify-between mt-8 pt-6 border-t border-gray-200">
          <FormButton
            ...
            className="hover:!bg-[#4F46E5] hover:!border-[#4F46E5] hover:!text-white"
          >
            Previous
          </FormButton>
...
// Step 2: Flock Details using shared components
  ...
  <div>
    ...
    >
      <TextInput
        ...
        onChange={(value: string) => onUpdate({ batchName: value })}
        placeholder="Initial Flock"
        required
      />

      {/* Bird Counts Grid */}
      <FormField label="Bird Counts">
        <div className="grid md:grid-cols-4 gap-4">
          <NumberInput
            label="Hens (laying females)"
            value={formData.henCount}
            onChange={(value: number) => onUpdate({ henCount: value })}
            min={0}
            placeholder="0"
          />

          <NumberInput
            label="Roosters (males)"
            value={formData.roosterCount}
            onChange={(value: number) => onUpdate({ roosterCount: value })}
            min={0}
            placeholder="0"
          />

          <NumberInput
            label="Chicks (young birds)"
            value={formData.chickCount}
            onChange={(value: number) => onUpdate({ chickCount: value })}
            min={0}
            placeholder="0"
          />

          <NumberInput
            label="Brooding (nesting females)"
            value={formData.broodingCount || 0}
            onChange={(value: number) => onUpdate({ broodingCount: value })}
            min={0}
            placeholder="0"
          />
        </div>
      ...
      <NeumorphicSelect
        ...
        onChange={(value: string) => onUpdate({ breed: value })}
        options={[
          { value: '', label: 'Select a breed...' },
          ...COMMON_BREEDS.map(breed => ({ value: breed, label: breed }))
        ]}
        required
      />

      <DateInput
        label="When did you get them?"
        value={formData.acquisitionDate}
        onChange={(value: string) => onUpdate({ acquisitionDate: value })}
        required
      />

      <NeumorphicSelect
        label="Age when you got them"
        value={formData.ageAtAcquisition || 'adult'}
        onChange={(value: string) => onUpdate({ ageAtAcquisition: value as 'chick' | 'juvenile' | 'adult' })}
        options={[
          { value: 'chick', label: 'Chicks (0-8 weeks)' },
          { value: 'juvenile', label: 'Juveniles (8-20 weeks)' },
          { value: 'adult', label: 'Adults (20+ weeks)' }
        ]}
        required
      />

      <TextInput
        label="Where did you get them from?"
        value={formData.source || 'onboarding'}
        onChange={(value: string) => onUpdate({ source: value })}
      ...
      <NumberInput
        ...
        onChange={(value: number) => onUpdate({ cost: value })}
        min={0}
        step={0.01}
        placeholder="0.00"
      />
